...
from Networks import NeuralCorefScorer
...
class nnCorefModel(pl.LightningModule):
    """
    ...
    def __init__(self,segment_max_num=args.segment_max_num,
        ...
        # -------------Model-Related--------------------------------
        self.CorefScorer = NeuralCorefScorer()
        # -------------Train-Related-------------------------------
        if args.bert_based:
    ...
    def shared_step(self, batch, stage):
        ...
        gold_indexes = torch.zeros_like(scores)
        ...
        for idx, span in enumerate(spans):
            ...
            if (span.i1, span.i2) in gold_mentions:
                ...
                if golds:
                    ...
                    found_corefs = sum((scores[idx, golds] > scores[idx, len(span.yi_idx)])).detach()
                    corefs_chosen += found_corefs.item()
                else:
                    # Otherwise, set gold to dummy
                    gold_indexes[idx, len(span.yi_idx)] = 1
                    
        loss = self.loss(scores, gold_indexes)
    ...
    def loss(self, scores, gold_indexes):
        eps = 1e-8
        loss = torch.sum(torch.log(torch.sum(torch.mul(scores, gold_indexes), dim=1).clamp_(eps, 1-eps)) * -1)
