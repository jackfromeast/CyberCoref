...
from Networks import CyberCorefScorer
from dataLoader import Corpus
from subprocess import Popen, PIPE

import numpy as np
from .typePredModel import typePredModel
...
class cyberCorefModel(pl.LightningModule):
    ...
    def __init__(self, MaxSentLen,
        ...
        self.CorefScorer = CyberCorefScorer(MaxSentLen, self.typePredictor)
        ...
        self.mention_detection_criterion = torch.nn.BCELoss(reduction="sum")
    ...
    def forward(self, doc):
        ...
        spans, coref_scores, all_spans, all_span_scores = self.CorefScorer.forward(doc)

        return spans, coref_scores, all_spans, all_span_scores
    ...
    def shared_step(self, batch, stage):
        ...
        spans, scores, all_spans, all_span_scores = self.forward(document)
        ...
        ### Select Training Samples for mention detection loss computation
        pred_gold_mentions = []
        true_labels = []
        for cur_span in all_spans:
            if (cur_span.i1, cur_span.i2) in gold_mentions:
                true_labels.append([0.0, 1.0])
                pred_gold_mentions.append(cur_span.si.tolist())
        true_labels = torch.tensor(true_labels).to(args.device)
        pred_gold_mentions = torch.tensor(pred_gold_mentions).to(args.device)

        # mention_loss = self.mention_detection_criterion(pred_gold_mentions, true_labels)

        coref_loss = self.softmax_loss(scores, gold_indexes)
        assert (not torch.isnan(coref_loss)), 'NaN Error!'
